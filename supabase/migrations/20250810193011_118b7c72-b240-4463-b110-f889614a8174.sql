-- Ensure driver_status enum includes canonical values
DO $$ BEGIN
...
-- Normalize legacy values in drivers (text column)
UPDATE public.drivers SET driver_status = 'busy' WHERE driver_status = 'on_job';
UPDATE public.drivers SET driver_status = 'offline' WHERE driver_status IN ('inactive', 'off_duty');

-- Normalize values in driver_status_history (enum column)

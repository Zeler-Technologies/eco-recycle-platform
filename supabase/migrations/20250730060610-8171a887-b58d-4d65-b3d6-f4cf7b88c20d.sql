-- Update create_tenant_complete function to work with mock authentication for development
-- This bypasses the auth check when no authenticated user exists (development mode)
...
BEGIN
  ...
  -- Check if user is super admin (only if we have an authenticated user)
  IF v_current_user_id IS NOT NULL THEN
    ...
    END IF;
  END IF;
  -- If no authenticated user (development/mock mode), allow creation

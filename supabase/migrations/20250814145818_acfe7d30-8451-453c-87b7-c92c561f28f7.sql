...
-- Pricing tiers policies (if table exists)
DO $$
BEGIN
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'pricing_tiers') THEN
        EXECUTE 'CREATE POLICY "pricing_tiers_super_admin_access" 
        ON public.pricing_tiers 
        FOR ALL 
        TO authenticated
        USING (public.is_super_admin_safe() = true)';

        EXECUTE 'CREATE POLICY "pricing_tiers_tenant_access" 
        ON public.pricing_tiers 
        FOR SELECT 
        TO authenticated
        USING (

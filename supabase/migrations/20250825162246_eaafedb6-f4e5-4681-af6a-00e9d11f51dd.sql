...
DROP FUNCTION IF EXISTS update_pickup_status_unified(uuid, text, text, text[]);
DROP FUNCTION IF EXISTS update_pickup_status_unified(uuid, text, text);

-- Create a single, clean 4-parameter function
CREATE FUNCTION update_pickup_status_unified(

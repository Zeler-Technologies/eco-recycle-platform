-- Helper is TEXT-based so comparisons never cast to the enum in RPCs
...
-- list_available_pickup_requests: use helper in NOT EXISTS filter
...
-- assign_driver_to_pickup: helper for active-exists, then insert + sync
...
-- (Perf/safety) Partial indexes with only immutable operators (no functions)
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_active_assignment_per_pickup
  ...
  WHERE (is_active IS DISTINCT FROM FALSE)
    ...
    AND status NOT IN ('completed'::public.assignment_status,
